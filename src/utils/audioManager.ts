...
class AudioManager {
  ...
  // Heartbeat Effect for Youssef's World (دقة قلب طول وقت السؤال)
  ...
  private playHeartbeatBeat() {
    ...
    // Second beat: Dub (slightly higher thump ~140ms later)
    const t2 = t + 0.14;
    const osc2 = this.ctx.createOscillator();
    ...
    gain2.gain.setValueAtTime(0.25 * this.sfxVolume, t2);
  ...
  // Realistic Clapping & Applause for Youssef's World correct answer (صقفة تفاعلية)
  public playApplause() {
    this.initContext();
    if (!this.ctx || !this.sfxGain || !this.sfxEnabled) return;
    const t = this.ctx.currentTime;

    // Generate repeated hand-clap bursts over 2.5 seconds
    const clapCount = 28;
    for (let i = 0; i < clapCount; i++) {
      const clapTime = t + (i * 0.08) + (Math.random() * 0.04);
      ...
      const gain = this.ctx.createGain();
      const vol = (0.18 + Math.random() * 0.15) * this.sfxVolume;
      gain.gain.setValueAtTime(vol, clapTime);
  ...
  // Gentle Sea Wind / Breeze Sound for Entrance Transition (صوت رياح وهدوء)
  public playWind(duration = 2.5) {
    this.initContext();
    if (!this.ctx || !this.sfxGain || !this.sfxEnabled) return;
    const t = this.ctx.currentTime;
    const bufferSize = Math.floor(this.ctx.sampleRate * duration);
    ...
    filter.frequency.exponentialRampToValueAtTime(850, t + duration * 0.5);
    filter.frequency.exponentialRampToValueAtTime(240, t + duration);

    const gain = this.ctx.createGain();
    gain.gain.setValueAtTime(0.001, t);
    gain.gain.linearRampToValueAtTime(0.35 * this.sfxVolume, t + 0.6);
  ...
  // Sheikh Hazza Al Balushi Quran Reciter Stream
  private quranAudio: HTMLAudioElement | null = null;
  private currentSurahIndex = 0;

  public playQuran(surahIndex = 0) {
    const surahs = [
      { num: 12, name: 'سورة يوسف', url: 'https://server11.mp3quran.net/hazza/012.mp3' },
      { num: 1, name: 'سورة الفاتحة', url: 'https://server11.mp3quran.net/hazza/001.mp3' },
      { num: 18, name: 'سورة الكهف', url: 'https://server11.mp3quran.net/hazza/018.mp3' },
      { num: 36, name: 'سورة يس', url: 'https://server11.mp3quran.net/hazza/036.mp3' },
      { num: 55, name: 'سورة الرحمن', url: 'https://server11.mp3quran.net/hazza/055.mp3' },
      { num: 67, name: 'سورة الملك', url: 'https://server11.mp3quran.net/hazza/067.mp3' },
    ];

    this.currentSurahIndex = surahIndex >= 0 && surahIndex < surahs.length ? surahIndex : 0;
    const targetSurah = surahs[this.currentSurahIndex];

    if (!this.quranAudio) {
      this.quranAudio = new Audio();
      this.quranAudio.crossOrigin = 'anonymous';
      this.quranAudio.addEventListener('ended', () => {
        // Play next surah
        this.playQuran((this.currentSurahIndex + 1) % surahs.length);
      });
    }

    this.quranAudio.src = targetSurah.url;
    this.quranAudio.volume = Math.min(1.0, this.musicVolume * 0.85);
    this.quranAudio.play().catch((err) => {
      console.log('Quran audio autoplay permission:', err);
    });
  }
